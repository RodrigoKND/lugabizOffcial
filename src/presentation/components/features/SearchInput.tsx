  ...
  return (
    <section ref={searchRef} className="relative">
      <header className="relative">
        ...
        <input type="search" placeholder="Buscar lugares..." value={value}
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="py-2">
              {suggestions.map((place, index) => (
                <motion.button key={place.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: index * 0.05 }}
                  onClick={() => { onChange(place.name); setIsOpen(false); onPlaceSelect(place); }}
                  className="w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors flex items-center space-x-3">
                  <img loading="lazy" src={place.image} alt={place.name} className="w-12 h-12 object-cover rounded-lg" />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-gray-900 truncate">{place.name}</h4>
                    <div className="flex items-center space-x-2 text-sm text-gray-500">
                      <MapPin className="w-3 h-3" /><span className="truncate">{place.address}</span>
                    </div>
                    <div className="flex items-center space-x-2 mt-1">
                      <div className={`px-2 py-1 rounded-full text-xs ${place.category?.color} text-white`}>{place.category?.name}</div>
                      {place.rating && <div className="flex items-center space-x-1"><Star className="w-3 h-3 fill-yellow-400 text-yellow-400" /><span className="text-xs text-gray-600">{place.rating}</span></div>}
                      {place.savedCount && place.savedCount > 0 && <div className="text-xs text-gray-500">{place.savedCount} guardados</div>}
                    </div>
                  </div>
                </motion.button>
              ))}
