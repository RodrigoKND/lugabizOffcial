...
import { MapMarker, MarkerContent } from '@/presentation/components/ui/map';
import type { OverpassElement } from '@/domain/entities';
...
const MapMarkers: React.FC<MapMarkersProps> = ({ filteredPlaces, onMarkerClick, searchQuery = "" }) => {
  const normalizedQuery = searchQuery.trim().toLowerCase();

  const markers = useMemo(() => {
    const grouped = new Map<string, OverpassElement[]>();
    
    filteredPlaces.filter((p) => p.lat != null && p.lon != null).forEach((place) => {
      const key = `${place.lat!.toFixed(4)}_${place.lon!.toFixed(4)}`;
      if (!grouped.has(key)) grouped.set(key, []);
      grouped.get(key)!.push(place);
    });

    const result: Array<{ place: OverpassElement; matches: boolean; offsetLat: number; offsetLon: number }> = [];
    ...
    grouped.forEach((places) => {
      places.forEach((place, index) => {
        ...
        const matches = !normalizedQuery || name.includes(normalizedQuery) || amenity.includes(normalizedQuery) ||
          tourism.includes(normalizedQuery) || cuisine.includes(normalizedQuery);

        const angle = (index * (360 / places.length)) * (Math.PI / 180);
        const radius = places.length > 1 ? 0.0001 : 0;
        
        result.push({ place, matches, offsetLat: Math.cos(angle) * radius, offsetLon: Math.sin(angle) * radius });
      });
    });
  ...
  return (
    <>
      {markers.map(({ place, matches, offsetLat, offsetLon }) => (
        <MapMarker key={place.id} longitude={place.lon! + offsetLon} latitude={place.lat! + offsetLat}
          onClick={() => onMarkerClick?.(place)} className="z-0 absolute">
          <MarkerContent className="!z-0">
            <div className="flex flex-col items-center relative">
              <div className={`px-2 py-1 rounded-md mb-1 whitespace-nowrap text-xs font-medium ${matches ? "bg-white text-gray-800 shadow-md" : "bg-gray-700 text-white"}`}>
                {place.tags?.name || "Sin nombre"}
              </div>
              <div className={`rounded-full border-2 border-white shadow-lg transition-all duration-200 cursor-pointer hover:scale-125 active:scale-95 flex items-center justify-center text-sm ${matches ? getPlaceColor(place) : "bg-gray-400"} ${matches ? "w-9 h-9" : "w-7 h-7"}`}>
                <span className="text-white">{getPlaceEmoji(place)}</span>
              </div>
            </div>
          </MarkerContent>
        </MapMarker>
