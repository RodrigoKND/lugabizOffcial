interface StoryCardProps {
  image?: string;
  name: string;
  onClick: () => void;
  isViewed?: boolean;
  // When true, no internal ring is rendered (parent StoriesRow handles it)
  externalRing?: boolean;
}

const StoryCard: React.FC<StoryCardProps> = ({ image, name, onClick, isViewed, externalRing }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1.5 shrink-0 group transition-opacity ${isViewed ? 'opacity-50' : ''}`}
  >
    {externalRing ? (
      // Ring is handled by StoriesRow — just show the avatar
      <div className="p-[2px] bg-white rounded-full">
        {image ? (
          <img
            src={image}
            alt={name}
            className={`w-16 h-16 rounded-full object-cover ${isViewed ? 'grayscale-[40%]' : ''}`}
          />
        ) : (
          <div className={`w-16 h-16 rounded-full flex items-center justify-center text-lg font-bold ${isViewed ? 'bg-stone-100 text-stone-400' : 'bg-primary-100 text-primary-500'}`}>
            {name.charAt(0)}
          </div>
        )}
      </div>
    ) : (
      // Standalone usage — render own ring
      <div className={`p-[2px] rounded-full group-active:scale-90 transition-transform ${isViewed ? 'bg-stone-300' : 'bg-gradient-to-br from-primary-400 via-pink-400 to-orange-400'}`}>
        <div className="p-[2px] bg-white rounded-full">
          {image ? (
            <img
              src={image}
              alt={name}
              className={`w-16 h-16 rounded-full object-cover ${isViewed ? 'grayscale-[40%]' : ''}`}
            />
          ) : (
            <div className={`w-16 h-16 rounded-full flex items-center justify-center text-lg font-bold ${isViewed ? 'bg-stone-100 text-stone-400' : 'bg-primary-100 text-primary-500'}`}>
              {name.charAt(0)}
            </div>
          )}
        </div>
      </div>
    )}
    <span className={`text-[11px] font-medium max-w-16 truncate text-center leading-tight ${isViewed ? 'text-stone-400' : 'text-text-secondary'}`}>
      {name}
    </span>
  </button>
);

export default StoryCard;
