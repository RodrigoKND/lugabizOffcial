...
import { DISTANCES_PLACES } from '@/constants/map';
...
const DistanceFilter: React.FC<DistanceFilterProps> = ({ selectedDistance, onDistanceChange }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      <div className={`absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 origin-top ${
        ...
        <ul className="py-2">
          {DISTANCES_PLACES.map((d, i) => (
            <li key={i} onClick={() => { onDistanceChange(d.value); setIsOpen(false); }}
              ...
              <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                selectedDistance === d.value ? 'border-white bg-white' : 'border-gray-300 bg-white'
              }`}>
                {selectedDistance === d.value && <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500" />}
              </div>
              <span className={`text-sm sm:text-base ${selectedDistance === d.value ? 'font-semibold' : 'font-medium'}`}>{d.name}</span>
            </li>
          ))}
        </ul>
      </div>
