...
import { PlaceCard } from '@presentation/components/features';
...
const PlacesResultsModal: React.FC<PlacesResultsModalProps> = ({
  isOpen, onClose, query, places, isLoading, onPlaceClick, onChatAgain
}) => {
  return (
      ...
      <form className="shadow-lg">
        ...
        <section className="container p-6 relative">
          <header className="absolute top-4 left-6 z-10">
            <button className="w-14 h-14 bg-gradient-to-b from-primary-500 to-tomato rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:shadow-xl transition-shadow"
              title="Chatear con Lubi" aria-label="Chatear con Lubi" onClick={onChatAgain} type="button">
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                <div className="inline-block animate-spin"><div className="w-8 h-8 border-4 border-purple-600 border-t-transparent rounded-full"></div></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 auto-rows-fr">
              {places.map((place) => <PlaceCard key={place.id} place={place} onClick={() => onPlaceClick(place.id)} />)}
            </div>
          ) : (
            <div className="text-center py-12"><p className="text-gray-500 text-lg">No se encontraron lugares para "{query}"</p></div>
          )}
        </section>
      </form>
    </Modal>
