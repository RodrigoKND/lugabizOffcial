...
const AllPlacesModal: React.FC<AllPlacesModalProps> = ({ isOpen, onClose }) => {
  ...
  const filteredPlaces = searchTerm.trim() ? searchPlaces(searchTerm) : places;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }}
          ...
          <header className="bg-linear-to-r from-primary-500 to-tomato p-6 relative">
            <button onClick={onClose} className="absolute cursor-pointer top-4 right-4 text-white/80 hover:text-white transition-colors">
              <X className="w-6 h-6" />
            </button>
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
            <div className="mb-6">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar por lugares, categorías o grupo social..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPlaces.length === 0 ? (
                ...
                filteredPlaces.map((place, index) => (
                  <motion.div key={place.id} initial={{ y: 30 }} animate={{ y: 0 }}
                    transition={{ delay: 0.05 * index }}>
                    <PlaceCard place={place} onClick={() => handlePlaceClick(place.id)} />
                  </motion.div>
                ))
              )}
            </div>
