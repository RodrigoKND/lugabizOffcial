      ...
      {places.length > 4 && (
        <div className="hidden lg:flex items-center gap-2 absolute -top-14 right-0">
          <button
            ...
            className={`p-2 rounded-full cursor-pointer shadow-lg transition-all ${canSlideLeft
          ...
          <button
            ...
            className={`p-2 rounded-full cursor-pointer shadow-lg transition-all ${canSlideRight
              ? 'hover:bg-gray-200 text-gray-900'
              : 'opacity-40 cursor-not-allowed text-gray-400'
              }`}
          >
          ...
          <button
            ...
            className="p-2 rounded-full cursor-pointer bg-linear-to-r from-primary-500 to-tomato text-white shadow-lg transition-all hover:scale-105">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4">
          {places.map((place: Place, index) => (
            <motion.div
              ...
              animate={{ x: 0 }}
              transition={{ delay: 0.05 * index }}
              className="snap-start shrink-0 w-[70vw] max-w-70"
            >
              <PlaceCard
                place={place}
                onClick={() => onPlaceClick(place)}
        ...
        <div className="hidden md:flex lg:hidden justify-center mt-6 gap-2">
          {Array.from({ length: totalPages }).map((_, idx) => (
            <button
              ...
              className={`h-2 cursor-pointer rounded-full transition-all ${currentPage === idx ? 'bg-primary-500 w-6' : 'bg-gray-300 w-2'
                }`}
            />
          ))}
        </div>
